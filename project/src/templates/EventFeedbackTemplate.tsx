...
const EventFeedbackTemplate: React.FC<Props> = ({ survey, previewMode = false }) => {
    ...
    <div className="ef-options">
      {q.options?.map((opt, i) => {
        ...
        <div key={i} className={`ef-option ${formData[q.id] === opt ? 'selected' : ''}`} onClick={() => handleAnswer(q.id, opt)}>
          ...
          {!previewMode ? (
            <motion.span className="ef-option-label" variants={aVariants} initial="initial" animate="animate">{wrapOptionLabel(opt, opt, q)}</motion.span>
          ) : (
            <span className="ef-option-label">{wrapOptionLabel(opt, opt, q)}</span>
          )}
        </div>
        );
      })}
  ...
  return (
    <div className="event-feedback-container">
      ...
      <div className="ef-powered">Powered by <a href="#">Pepperwahl</a></div>
      {submitted && (
