 ...
 *
 * Two-panel modal mirroring PublishToMoustacheModal:
 ...
 *
 * Calls: POST /api/admin/surveys/<id>/publish-to-surveyforever
 * API endpoint / key updated once Survey Forever docs are shared.
 */
...
interface Props {
  ...
  onClose: () => void;
  onPublished: (sfSurveyId: string, status: string) => void;
}

// ─── Component ────────────────────────────────────────────────────────────────

const PublishToSurveyForeverModal: React.FC<Props> = ({
  surveyShortId,
  surveyTitle,
  existingSfId,
  existingQuestions,
  ...
  const [result, setResult] = useState<{ success: boolean; message: string; sfId?: string; payloadSent?: any } | null>(null);
  ...
  const buildPreviewPayload = () => {
    ...
    const preview: any = {
      ...
      survey_link: `https://survey.pepperwahl.com/survey/${surveyShortId}?uid={{user_id}}&src=surveyforever`,
  ...
  const generateDescription = async () => {
    ...
    try {
      ...
      if (data.description) setExtra(x => ({ ...x, description: data.description }));
    } catch { /* silent */ }
  ...
  const handlePublish = async () => {
    ...
    try {
      const token = localStorage.getItem('auth_token');
      const res = await fetch(`${baseUrl}/api/admin/surveys/${surveyShortId}/publish-to-surveyforever`, {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
      ...
      const data = await res.json();
      if (data.success) {
        setResult({ success: true, message: data.message, sfId: data.sf_survey_id, payloadSent: data.payload_sent });
        onPublished(data.sf_survey_id, data.status);
      } else {
  ...
  // ─── Shared styles (identical to Moustache modal) ─────────────────────────

  const C = {
    overlay: { position: 'fixed' as const, inset: 0, background: 'rgba(10,20,40,0.55)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 },
    modal: { background: '#FDFCFA', borderRadius: 18, width: '100%', maxWidth: 940, height: '90vh', maxHeight: '90vh', display: 'flex', flexDirection: 'column' as const, boxShadow: '0 24px 70px rgba(10,20,40,0.3)', border: '1px solid #EBE8E3', overflow: 'hidden', fontFamily: "'Outfit', -apple-system, sans-serif" },
    header: { padding: '16px 24px', borderBottom: '1px solid #EBE8E3', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexShrink: 0 },
    body: { flex: '1 1 0', display: 'flex', overflow: 'hidden', minHeight: 0, height: 0 },
    panel: { flex: '1 1 0', overflowY: 'auto' as const, padding: '20px 22px', display: 'flex', flexDirection: 'column' as const, gap: 12, scrollbarWidth: 'none' as const, minHeight: 0, paddingBottom: 24 },
    ...
    cardHead: (expanded: boolean) => ({ padding: '10px 14px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, cursor: 'pointer', background: expanded ? '#F0F7FF' : '#FDFCFA', borderBottom: expanded ? '1px solid #EBE8E3' : 'none' }),
    ...
    pill: (active: boolean) => ({ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '3px 9px', borderRadius: 20, fontSize: 10.5, fontWeight: 700, cursor: 'pointer', border: `1px solid ${active ? '#10B981' : '#EBE8E3'}`, background: active ? '#ECFDF5' : '#F5F1E8', color: active ? '#059669' : '#9B9189', transition: 'all 0.1s', userSelect: 'none' as const, flexShrink: 0 }),
    ...
    btnPrimary: (disabled: boolean) => ({ display: 'flex', alignItems: 'center', gap: 7, background: disabled ? '#C8D8E8' : 'linear-gradient(135deg, #1565C0 0%, #0D47A1 100%)', color: disabled ? '#8AA0B8' : '#fff', border: 'none', borderRadius: 10, padding: '9px 22px', fontSize: 13, fontWeight: 700, cursor: disabled ? 'not-allowed' : 'pointer', fontFamily: 'inherit', boxShadow: disabled ? 'none' : '0 2px 8px rgba(21,101,192,0.35)' }),
    addBtn: { display: 'flex', alignItems: 'center', gap: 6, background: '#F0F7FF', border: '1px dashed #90CAF9', borderRadius: 9, padding: '7px 12px', fontSize: 11.5, fontWeight: 600, color: '#1565C0', cursor: 'pointer', fontFamily: 'inherit', width: '100%', justifyContent: 'center' },
      ...
      <div style={C.modal}>
        ...
        <div style={C.header}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <div style={{ width: 36, height: 36, borderRadius: 10, background: 'linear-gradient(135deg, #1565C0 0%, #0D47A1 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
              <span style={{ color: '#fff', fontSize: 13, fontWeight: 900 }}>SF</span>
            </div>
            <div>
              <p style={{ fontSize: 15, fontWeight: 700, color: '#2D2520', margin: 0 }}>
                {existingSfId ? 'Update on Survey Forever' : 'Publish to Survey Forever'}
              </p>
              <p style={{ fontSize: 11, color: '#9B9189', margin: 0 }}>{surveyTitle}</p>
            </div>
            {existingSfId && (
              <div style={{ display: 'inline-flex', alignItems: 'center', gap: 5, background: '#EFF8FF', border: '1px solid #BAE6FD', borderRadius: 8, padding: '3px 10px', marginLeft: 4 }}>
                <span style={{ width: 6, height: 6, borderRadius: '50%', background: '#0284C7', display: 'inline-block' }} />
                <span style={{ fontSize: 10, color: '#0369A1', fontWeight: 600 }}>Live · {existingSfId}</span>
              </div>
            )}
          </div>
        ...
        <div style={C.body}>
          ...
          <div style={{ ...C.panel, maxWidth: 500 }}>
            <div>
              <p style={C.sectionTitle}>Pre-screening questions</p>
              <p style={{ fontSize: 11, color: '#9B9189', margin: 0 }}>
                Users on Survey Forever answer these first. Only those who pick a <span style={{ color: '#059669', fontWeight: 700 }}>qualifying</span> answer on every question are forwarded to your survey.
              </p>
            </div>

            {/* Template picker */}
            <div style={{ border: '1px solid #EBE8E3', borderRadius: 11, overflow: 'hidden' }}>
              <div style={{ padding: '9px 14px', background: '#F0F7FF', borderBottom: '1px solid #EBE8E3', display: 'flex', alignItems: 'center', gap: 7 }}>
                <LayoutTemplate size={12} color="#1565C0" />
                <span style={{ fontSize: 11, fontWeight: 700, color: '#1565C0', textTransform: 'uppercase' as const, letterSpacing: '0.07em' }}>Load a preset template</span>
                <span style={{ fontSize: 10, color: '#90CAF9', marginLeft: 4 }}>— replaces current questions, then edit freely</span>
              </div>
              <div style={{ padding: '10px 12px', display: 'flex', flexWrap: 'wrap' as const, gap: 7 }}>
                {TEMPLATES.map(t => (
                  <button
                    ...
                    style={{ fontSize: 11, fontWeight: 600, padding: '5px 11px', borderRadius: 20, border: '1px solid #EBE8E3', background: '#FDFCFA', color: '#6B6158', cursor: 'pointer', fontFamily: 'inherit', transition: 'all 0.1s' }}
                    onMouseEnter={e => { (e.currentTarget as HTMLButtonElement).style.background = '#EFF8FF'; (e.currentTarget as HTMLButtonElement).style.borderColor = '#1565C0'; (e.currentTarget as HTMLButtonElement).style.color = '#1565C0'; }}
                ...
                <button
                  ...
                  style={{ fontSize: 11, fontWeight: 600, padding: '5px 11px', borderRadius: 20, border: '1px dashed #90CAF9', background: '#F0F7FF', color: '#1565C0', cursor: 'pointer', fontFamily: 'inherit' }}
                >
                  + Blank
                </button>
              </div>
              ...
              <div key={qIdx} style={C.card}>
                <div style={C.cardHead(expandedIdx === qIdx)} onClick={() => setExpandedIdx(expandedIdx === qIdx ? -1 : qIdx)}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, minWidth: 0 }}>
                    <span style={{ width: 20, height: 20, borderRadius: 5, background: '#1565C0', color: '#fff', fontSize: 10, fontWeight: 800, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                      {qIdx + 1}
                    </span>
                    <span style={{ fontSize: 12.5, fontWeight: 600, color: q.question.trim() ? '#2D2520' : '#90CAF9', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 5, flexShrink: 0 }}>
                    ...
                    {questions.length > 1 && (
                      <button onClick={e => { e.stopPropagation(); removeQuestion(qIdx); }} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#90CAF9', display: 'flex', padding: 1 }}>
                        <Trash2 size={12} />
                      </button>
                    )}
                ...
                {expandedIdx === qIdx && (
                  <div style={C.cardBody}>
                    <div>
                      ...
                      <input style={C.input} placeholder="e.g. Do you shop online regularly?" value={q.question} onChange={e => updateQuestion(qIdx, e.target.value)} />
                    </div>
                    <div>
                        ...
                        {q.options.map((opt, oIdx) => {
                          ...
                          return (
                            <div key={oIdx} style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
                              ...
                              {q.options.length > 2 && (
                                <button onClick={() => removeOption(qIdx, oIdx)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#90CAF9', display: 'flex', padding: 1, flexShrink: 0 }}>
                                  <X size={12} />
                                </button>
                              )}
                            </div>
          ...
          <div style={{ ...C.panel, minWidth: 280, maxWidth: 360 }}>
            <div>
              <p style={C.sectionTitle}>Survey details</p>
              <p style={{ fontSize: 11, color: '#9B9189', margin: 0 }}>
                These are sent to Survey Forever for targeting and reward setup.
              </p>
            </div>

            {/* Description */}
            <div>
              <label style={C.label}><span style={{ display: 'flex', alignItems: 'center', gap: 5 }}><FileText size={11} /> Description</span></label>
              <div style={{ position: 'relative' }}>
                <textarea
                  style={{ ...C.input, resize: 'vertical' as const, minHeight: 68, paddingRight: 82 }}
                  placeholder="Describe this survey for Survey Forever users…"
                ...
                <button
                  ...
                  style={{ position: 'absolute', top: 7, right: 7, display: 'flex', alignItems: 'center', gap: 4, background: generatingDesc ? '#C8D8E8' : 'linear-gradient(135deg, #7C3AED, #5B21B6)', color: '#fff', border: 'none', borderRadius: 6, padding: '4px 9px', fontSize: 10, fontWeight: 700, cursor: generatingDesc ? 'not-allowed' : 'pointer', fontFamily: 'inherit' }}
                >
                  {generatingDesc ? <><Loader size={10} style={{ animation: 'spin 1s linear infinite' }} /> Generating…</> : <><Sparkles size={10} /> AI Generate</>}
                </button>
              </div>
              <p style={{ fontSize: 10, color: '#90CAF9', margin: '3px 0 0' }}>Auto-generated from survey title if empty</p>
            </div>

            {/* Payout */}
            <div>
              <label style={C.label}><span style={{ display: 'flex', alignItems: 'center', gap: 5 }}><DollarSign size={11} /> Payout (USD)</span></label>
              ...
              <p style={{ fontSize: 10, color: '#90CAF9', margin: '3px 0 0' }}>Sent as <code style={{ fontSize: 10 }}>payout_usd</code> · defaults to 0 if blank</p>
            </div>

            {/* Survey Type */}
            <div>
              <label style={C.label}><span style={{ display: 'flex', alignItems: 'center', gap: 5 }}><Tag size={11} /> Survey type</span></label>
            ...
            {/* Country */}
            <div>
              <label style={C.label}><span style={{ display: 'flex', alignItems: 'center', gap: 5 }}><Globe size={11} /> Country</span></label>
              <input style={C.input} placeholder="e.g. US, IN, GB" value={extra.country} onChange={e => setExtra(x => ({ ...x, country: e.target.value }))} />
              <p style={{ fontSize: 10, color: '#90CAF9', margin: '3px 0 0' }}>2-letter ISO code</p>
            </div>

            {/* Age range */}
            <div>
              <label style={C.label}><span style={{ display: 'flex', alignItems: 'center', gap: 5 }}><Users size={11} /> Target age range</span></label>
            ...
            {/* LOI */}
            <div>
              <label style={C.label}><span style={{ display: 'flex', alignItems: 'center', gap: 5 }}><Clock size={11} /> LOI (minutes)</span></label>
            ...
            {/* Notes */}
            <div>
              <label style={C.label}><span style={{ display: 'flex', alignItems: 'center', gap: 5 }}><FileText size={11} /> Notes (internal)</span></label>
              <textarea style={{ ...C.input, resize: 'vertical' as const, minHeight: 60 }} placeholder="Any instructions or context for Survey Forever…" value={extra.notes} onChange={e => setExtra(x => ({ ...x, notes: e.target.value }))} />
            </div>

            {/* Expiry date */}
            <div>
              <label style={C.label}><span style={{ display: 'flex', alignItems: 'center', gap: 5 }}><Calendar size={11} /> Expiry date</span></label>
              <input style={C.input} type="date" value={extra.expiry_date} min={new Date().toISOString().split('T')[0]} onChange={e => setExtra(x => ({ ...x, expiry_date: e.target.value }))} />
            </div>
            ...
            <div style={{ border: '1px solid #EBE8E3', borderRadius: 10, overflow: 'hidden' }}>
              <button
                ...
                style={{ width: '100%', padding: '9px 13px', background: showJsonPreview ? '#EFF8FF' : '#F9F7F4', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'space-between', fontFamily: 'inherit' }}
              >
                <span style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 11, fontWeight: 700, color: '#4B5563' }}>
                  <Code size={12} color="#1565C0" /> Preview JSON payload
                </span>
                <span style={{ fontSize: 10, color: '#9B9189' }}>{showJsonPreview ? 'Hide' : 'Show what will be sent'}</span>
              </button>
              {showJsonPreview && (() => {
                ...
                return (
                  <div style={{ position: 'relative', background: '#1E1E2E' }}>
                    <button
                      ...
                      style={{ position: 'absolute', top: 8, right: 8, display: 'flex', alignItems: 'center', gap: 4, background: jsonCopied ? '#059669' : '#374151', color: '#fff', border: 'none', borderRadius: 6, padding: '3px 8px', fontSize: 10, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}
                    >
                      {jsonCopied ? '✓ Copied' : 'Copy'}
                    </button>
                    <pre style={{ margin: 0, padding: '14px 14px 14px 14px', fontSize: 10.5, color: '#E2E8F0', overflowX: 'auto', lineHeight: 1.6, maxHeight: 260, overflowY: 'auto' }}>
                      {jsonStr}
                    </pre>
                  </div>
                );
          ...
          <div style={{ flex: 1 }}>
            {result && (
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8 }}>
                ...
                <div>
                  <p style={{ fontSize: 12, fontWeight: 600, color: result.success ? '#059669' : '#DC2626', margin: 0 }}>{result.message}</p>
                  {result.sfId && <p style={{ fontSize: 10, color: '#9B9189', margin: '2px 0 0' }}>SF ID: {result.sfId}</p>}
                </div>
              </div>
            )}
          </div>
          <div style={{ display: 'flex', gap: 8, flexShrink: 0 }}>
            ...
            <button style={C.btnPrimary(publishing)} onClick={handlePublish} disabled={publishing}>
              {publishing
                ...
                : <><Send size={13} /> {existingSfId ? 'Update on Survey Forever' : 'Publish to Survey Forever'}</>
              }
            </button>
          </div>
        </div>
...
export default PublishToSurveyForeverModal;
